...
\begin{frame}{Access the Website}
...

  {
    ...
    \href{\WebsiteURL}{\textcolor{blue}{\WebsiteURL}}
  }

  \vspace{1cm}

  \qrcode[height=4cm]{\WebsiteURL}
...
\note{
  This is how GecWeb when running on a desktop browser.
  As you can see, the corrections are highlighted in green.
}
...
\note{
  And you can click on the highlighted text, you will see a simple explanation about the correction.
}
...
\note{
  Additionally, GecWeb has a dynamic layout that can adapt to different screen sizes.
  On mobile devices, the output text box is moved down to the bottom of the screen as shown in the image.

}

\begin{frame}{Data usage}
  \begin{figure}
    \includegraphics[width=0.9\textwidth]{data-usage}
    \caption{GecWeb data usage}
  \end{figure}
\end{frame}

\note{
  And last but not least, GecWeb is lightweight: it only requires 8 KB of data transfer for each run.
  This means that GecWeb can can be easily used with a slow internet connections.
}
